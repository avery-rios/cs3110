...
if [ -e a3src.zip ]; then
  echo "a3src.zip exists.  Good."
  if zipinfo -1 a3src.zip | grep -q "^data.ml$"; then
    echo "data.ml is in the root folder of the zip.  Good."
  else 
    echo "data.ml is NOT in the root folder of the zip.  This is bad."
    echo "Did you create the zip by running \"make zip\"?"
  zip=bad
fi
else
  echo "a3src.zip does NOT exist.  This is bad.  Have you run \"make zip\"?"
  zip=bad
fi
